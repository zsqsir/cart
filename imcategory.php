<?php
//require_once 'DetectEncoding.php';
//echo detect_encoding('再根据一些非utf8编码的情');

// DB
define('DB_DRIVER', 'mysqli');
define('DB_HOSTNAME', 'localhost');
define('DB_USERNAME', 'root');
define('DB_PASSWORD', '');
define('DB_DATABASE', 'xy');
define('DB_PORT', '3306');
define('DB_PREFIX', 'mcc_');

// DB2
define('TDB_DRIVER', 'mysqli');
define('TDB_HOSTNAME', '192.168.1.83');
define('TDB_USERNAME', 'zlf');
define('TDB_PASSWORD', '');
define('TDB_DATABASE', 'a');
define('TDB_PORT', '3306');
define('TDB_PREFIX', '');
//var_dump(get_defined_constants());
require_once 'system/library/db/mysqli.php';
$newdb = new DB\MySQLi(DB_HOSTNAME, DB_USERNAME, DB_PASSWORD, DB_DATABASE);
$olddb = new DB\MySQLi(TDB_HOSTNAME, TDB_USERNAME, TDB_PASSWORD, TDB_DATABASE);
$rs=$olddb->query("select * from dkind where nid order by nid");
foreach ($rs->rows as $row) {

    $des = $row['des'];
    $nid = $row['nid'];

//    $sql = "INSERT INTO " . DB_PREFIX . "category SET category_id=$nid, parent_id = '" . (int)$row['pid'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . "', sort_order = '" . "', status = '1" .  "', date_modified = NOW(), date_added = NOW()";
//    dealsql($sql);
//
//    $sql = "INSERT INTO " . DB_PREFIX . "category_description SET category_id = '" . (int)$nid . "', language_id = '1" . "', name = '" . $des . "', description = '" . $des . "', meta_title = '" . $des . "', meta_description = '" . $des. "', meta_keyword = '" .  "'";
//    dealsql($sql);

    $sql = "INSERT INTO " . DB_PREFIX . "category_to_store SET category_id = '" . (int)$nid . "', store_id = '" ."'";
    dealsql($sql);
    continue;

}

function dealsql($sql)
{
    $sql = iconv('GBK','UTF-8',$sql);
    echo '<p>'.$sql;
    global $newdb;
    echo $newdb->query($sql);

}

