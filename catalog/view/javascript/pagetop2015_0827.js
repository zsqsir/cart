    ...
    if(isShowClass){ 
        if(jsondata[1]&& jsondata[1].length>0){
            ...
            for(var j=0;j< childs ;j++){
                if(childdata[j][3]=="1") {
                    result_class+="<li type=\"class\" id=\"li_"+d+"\" value=\"" +d+ "\" onclick=\"goto_class('"+childdata[j][2]+"','#topkeys#',"+d+",'"+all_keys+"','searchinput')\" onmouseover=\"mo(this.value)\"><span></span>在<strong>"+childdata[j][1]+"</strong>中搜</li>";
                }else {
                    result_class+="<li type=\"class\" id=\"li_"+d+"\" value=\""+d+ "\" onclick=\"goto_class('"+childdata[j][0]+"','#topkeys#',"+d+",'"+all_keys+"','searchinput')\" onmouseover=\"mo(this.value)\"><span></span>在<strong>"+childdata[j][1]+"</strong>中搜</li>";
                }
                d++;
            }
...
function goto_class(classstr,key,pos,allkeys,act){

    sug_gid('suggest_class_btn').click();
    head_search_data(act,classstr,key,pos,allkeys);
    if(classstr>=4000000)
  {
        location.href="http://search.dangdang.com/?key=" + key + "&category_id="+classstr + "&ref=suggest-1-1&act=input";
        return ;
  }
    location.href="http://search.dangdang.com/?key=" + key + "&category_path=" + classstr + "&SearchFromTop=1&ref=suggest-1-1&act=input";
    return;
}
...
function initHeaderOperate()
{
    ...
    if(aspxauth==''){
        xinshou = '<a dd_name="登录" href="javascript:PageTopLogIn();" target="_self" rel="nofollow" class="login_link">登录</a><a dd_name="免费注册" href="javascript:PageTopRegist();" target="_self" rel="nofollw">免费注册</a>';
...
function AddFavorite(url,title){
    ...
    if(title==""){
        title = "当当—网上购物中心：图书、母婴、美妆、家居、数码、家电、服装、鞋包等，正品低价，货到付款";
    }
        try{
